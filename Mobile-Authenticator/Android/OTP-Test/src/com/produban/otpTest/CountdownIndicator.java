...
public class CountdownIndicator extends View {
	...
	private static final int DEFAULT_COLOR = 0xae000000;

	/**
	 * Countdown phase starting with {@code 1} when a full cycle is remaining
	 * and shrinking to {@code 0} the closer the countdown is to zero.
	 */
	...
	public CountdownIndicator(Context context, AttributeSet attrs) {
		...
		TypedArray appearance = theme.obtainStyledAttributes(attrs,
	...
	protected void onDraw(Canvas canvas) {
		...
		// We need to leave the leftmost column and the topmost row out of the
		// drawingRect because
		// in anti-aliased mode drawArc and drawOval use these areas for some
		// reason.
		...
		if (remainingSectorStartAngle < 360) {
			canvas.drawArc(drawingRect, remainingSectorStartAngle,
					remainingSectorSweepAngle, true, mRemainingSectorPaint);
		} else {
			// 360 degrees is equivalent to 0 degrees for drawArc, hence the
			// drawOval below.
	 ...
	 * 
	 * @param phase
	 *            phase {@code [0, 1]}: {@code 1} when the maximum amount of
	 *            time is remaining, {@code 0} when no time is remaining.
	 */
